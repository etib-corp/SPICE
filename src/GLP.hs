...
testParserConfiguration :: String -> IO String
testParserConfiguration content = case getParserConfiguration content of
    Nothing -> pure $ "error"
    Just config -> case parse "(if(eq? a b) (lol a) (lolb))" (parseExpressionConfig config) of
        Left err -> pure $ show err
        Right result -> pure $ show result

loadParserConfiguration :: String -> IO String
